...
def forecast(address):
    ...
    try:
        code = '你的氣象資料授權碼'
        url = f'https://opendata.cwb.gov.tw/fileapi/v1/opendataapi/{code}&downloadType=WEB&format=JSON'
        ...
        for i in area_list: 
            if i in address:        # 如果使用者的地址包含縣市名稱
                ...
                # 將進一步的預報網址換成對應的預報網址
                url = f'https://opendata.cwb.gov.tw/api/v1/rest/datastore/{json_api[i]}?Authorization=CWB-DCA7061B-EEEC-496E-9B46-DC7549FA5F88&elementName=WeatherDescription'
